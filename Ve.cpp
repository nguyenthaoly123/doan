...
void DatVe(PTR &First, NODEPTR &root) {
    ...
    while (true) {
        ConstraintForOnlyNumber(cmnd, ordinal, SaveOrNot, 15, 999999999);
        if (SaveOrNot) {
    ...
    while (true) {
        ...
        ConstraintForOnlyNumber(soGheNhap, ordinal, SaveOrNot, 15, 999999999);
        if (SaveOrNot) {
...
void MenuVe() {
    ...
    while (true) {
        if (menuCap == 1) {
            ...
            InVeTrong();  // Bo comment neu da merge vao cung file
            system("cls"); ShowCur(false);
            VeKhung(); VeKhungHuongDan();
...
static void InDanhSachVeTrong(CB &cb, int startRow)
{
    ...
    for (int i = startRow; i <= BOT-2; i++)
        ClearArea(L_LEFT+1, i, L_RIGHT-1, 1);

    gotoXY(L_LEFT+1, startRow); SetColor(14);
    cout << "VE TRONG (" << (int)vecTrong.size() << "):"; SetColor(7);

    if (vecTrong.empty()) {
        gotoXY(L_LEFT+1, startRow+1); SetColor(12);
        cout << "  Het ve!"; SetColor(7);
        return;
    }

    int dataStart = startRow + 1;
    int dataEnd   = BOT - 2;
    int availRows = dataEnd - dataStart + 1;
    if(availRows < 1) availRows = 1;

    int frameW  = L_RIGHT - L_LEFT - 2;
    int colW    = (cb.tongsove >= 1000) ? 5 : 4;
    int maxCols = frameW / colW;
    if(maxCols < 1) maxCols = 1;

    int cols = ((int)vecTrong.size() + availRows - 1) / availRows;
    if(cols < 1)     cols = 1;
    ...
    for (int i = 0; i < (int)vecTrong.size(); i++) {
        ...
        int col = i % cols;
        int y   = dataStart + row;
        int x   = L_LEFT + 1 + col * colW;

        if(y > dataEnd) {
            gotoXY(L_LEFT+1, dataEnd); SetColor(12);
            ostringstream oss;
            oss << "+" << ((int)vecTrong.size() - i) << " ghe...";
            cout << left << setw(frameW) << oss.str();
            SetColor(7);
            break;
        }
        ...
        gotoXY(x, y);
        SetColor(10);
        cout << setw(colW) << right << vecTrong[i];
        SetColor(7);
    }
}
...
static void InDSChuyenBay(vector<PTR> &dsCB, int highlight, int startIdx)
{
    ...
    for (int i = 0; i+startIdx < (int)dsCB.size() && i < PER_PAGE; i++) {
        ...
        cout << setw(5) << cb.tongsove
             << setw(8) << TrangThaiStr(cb.trang_thai);
        SetColor(7);
    }
...
static void BuocXemVeTrong(CB &cb)
{
    ...
    gotoXY(L_LEFT+1, 12); SetColor(8);
    for (int i = 0; i < L_RIGHT-1; i++) cout << (char)196;
    SetColor(7);

    InDanhSachVeTrong(cb, 13);
