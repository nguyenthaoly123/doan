...
static int SNgayGio(PTR First, DSMB LA)
{
    ...
    while (true) {
        ...
        // 4. NH?N ENTER Ð? CH?N S?A
            ...
            if (dongYSua) {
                // 1. Sao luu l?i ngày gi? cu tru?c khi s?a
                Datetime oldNgayGio = x.thoigiandi; 
                
                ShowCur(true);
                
                // 2. VÒNG L?P B?T BU?C NH?P L?I N?U SAI
                while (true) {
                    ...
                    if (!kiemtrahople(x.thoigiandi)) { 
                        ...
                        continue; // B?m phím b?t k? xong s? quay l?i d?u vòng l?p while d? nh?p l?i
                    }
                    else if(!ConHon1Tuan(x.thoigiandi)){
				        gotoXY(2,14); SetColor(12);cout<< "Phai dieu chinh truoc it nhat 1 tuan";
	                 	_getch();
	        	       ClearArea(2, 14, 36, 1);
	        	       x.thoigiandi = oldNgayGio;
	        	       continue;
